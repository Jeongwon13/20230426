...
public class InitFilter extends HttpFilter implements Filter {
	...
	public void init(FilterConfig fConfig) throws ServletException {
		// logger를 이용해 출력하는 방법
		// trace - debug - info - warn - error 범위
		// 보통은 debug, info, error만 주로 사용함.
		
		// debug: 개발의 흐름 파악할 때(이 부분이(해당코드가) 실행되었는 지, 파라미터 현재 무슨 값인지 확인할 때 등)
		// info: 메소드가 실행되었는지 파악할 때 주로 사용
		 
		logger.info("초기화 필터 생성");
	}

	// 필터가 파괴될 때 실행(서버는 켜져 있는데, 백엔드 코드 수정되었을 때)
	public void destroy() {
